...
```{r}
table(cut(diabetes$time_in_hospital, seq.int(1,100,9)))
...
```{r}
library(dataMaid)
makeDataReport(diabetes[,c(6:18)])
```
...
```{r}
barplot(table1,cex.names=.7,cex.axis = .7)
```
...
and male.
```{r}
barplot(100*prop.table(table2), legend=TRUE, main=" \n by Gender", 
        beside=TRUE, 
        ylim=c(0,80), 
        ylab="Percent",
