...
```{r}
library(psych)
psych::describe(chi[,2:4]) 
#this function on the categorical does not provice useful information on the categorical variable
```

# using describe function from another package is useful
```{r}
library(Hmisc)
Hmisc::describe(chi[,2:4])
```
...
CrossTable(chi$treatment, chi$improvement, chisq = TRUE, format='SAS') #can also use a SPSS format
```
